...
<head>
    <meta charset="UTF-8">
    <title>Reporte de Asistencia Acumulado</title>
    <style>
...
<body>
    <h2>Reporte de Asistencia Acumulado</h2>
    <div class="subtitle">
        @if($user)
            Empleado: {{ $user->name }}
        @else
        ...
        <div class="period-list">
            @foreach($periods as $p)
                <span>{{ $p['start']->format('d/m/Y') }} - {{ $p['end']->format('d/m/Y') }}</span>
            @endforeach
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="10" style="text-align: center;">No hay registros para los períodos seleccionados.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
