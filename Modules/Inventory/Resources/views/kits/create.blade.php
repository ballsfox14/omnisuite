<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('inventory.kits.store') }}" id="kitForm">
                        @csrf
                        ...
                        <div class="mb-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
                            ...
                            <div class="overflow-x-auto">
                                ...
                                <p class="text-sm text-gray-500 mt-2" id="no-tools-message">No hay herramientas
                                    agregadas.</p>
                            </div>
                        ...
                        <div class="flex justify-end gap-2">
                            ...
                            <button type="submit"
                                ...
                                <span class="material-icons">save</span> Guardar Kit
                            </button>
                        </div>
                    </form>
                </div>
    ...
    @push('scripts')
        <script>
            // Datos de herramientas para usar en JS
            ...
            document.getElementById('add-tool-btn').addEventListener('click', function () {
                ...
                // Verificar si ya fue agregada (opcional)
                // Podrías permitir duplicados, pero si no, puedes verificar aquí
                ...
                hiddenId.name = `tools[${toolIndex}][id]`;
                hiddenId.value = toolId;
                hiddenContainer.appendChild(hiddenId);

                const hiddenQty = document.createElement('input');
                hiddenQty.type = 'hidden';
                hiddenQty.name = `tools[${toolIndex}][quantity]`;
            ...
            // Eliminar herramienta de la tabla y sus campos ocultos
            document.addEventListener('click', function (e) {
                if (e.target.closest('.remove-tool')) {
                    const row = e.target.closest('tr');
                    const index = Array.from(tableBody.children).indexOf(row);
                    // Eliminar los campos ocultos correspondientes (hay 2 por fila)
                    // Los campos ocultos se agregaron en orden, así que podemos eliminarlos por índice
                    const hiddenInputs = hiddenContainer.children;
                    // Eliminar dos elementos (id y quantity) por cada fila eliminada
                    // Como es más complejo, mejor regeneramos los campos ocultos desde las filas restantes
                    row.remove();

                    // Regenerar campos ocultos desde las filas actuales
